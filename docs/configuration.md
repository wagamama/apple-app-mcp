...
This page covers the Mac Mail MCP config surface and the Mac Calendar MCP
config surface. Both packages support environment variables and an optional
TOML config file generated by their `init` command.
...
Mac Calendar MCP can be configured via a TOML config file at
`~/.mac-calendar-mcp/config.toml`, environment variables, or both.
...
This writes `~/.mac-calendar-mcp/config.toml` with every available key
documented inline. All values are commented out, so defaults remain in effect.
...
Calendar config precedence is environment variables, then TOML config file,
then built-in defaults. The generated `config.toml` is written with `0600`
permissions.
