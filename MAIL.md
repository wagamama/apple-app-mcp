# Mac Mail MCP - Domain Instructions

Mail-specific architecture, tools, workflows, and caveats for this repository.
Read this file after `AGENTS.md` when working on the Mac Mail MCP server.

## Project Overview

The only Mac Mail MCP server with full-coverage FTS5 body search. Reliable on large mailboxes (tested at ~73K messages) where AppleScript-based servers timeout, and the only one whose body search has no recency cap. Disk-first email reads (~3ms via .emlx parsing), batch JXA property fetching, and an FTS5 search index for full-text body search (~28ms).
...
| **Disk (Batch)** | Indexing, sync | ~15ms/100 emails | startup, `mac-mail-mcp index` |
...
mac-mail-mcp init         # Write a commented config.toml template
...
1. CLI flag (e.g. `mac-mail-mcp serve -r`)
2. Environment variable (`APPLE_MAIL_*`)
3. `~/.mac-mail-mcp/config.toml` (TOML, schema v1 — generated by `mac-mail-mcp init`)
4. Built-in default
...
the `mac-mail-mcp init` template). Malformed TOML, unknown keys, type
...
| `APPLE_MAIL_INDEX_PATH` | `[index] path` | `~/.mac-mail-mcp/index.db` | Index database location |
...
Competitive benchmarks live in `benchmarks/` and compare against 6 other Mac Mail MCP servers.
...
- `benchmarks/setup.sh` — Install all competitors to `~/.cache/mac-mail-mcp-bench/`
